...
public:
  ...
  uint64_t reserve(int64_t reserved);
  ...
  const uint64_t& filled_qty() const;

  /// @brief get the total filled cost of this order
  const uint64_t& filled_cost() const;
  ...
  void replace(uint64_t size_delta, uint64_t new_price);
  ...
  void modify(int64_t size_delta, uint64_t new_price);
